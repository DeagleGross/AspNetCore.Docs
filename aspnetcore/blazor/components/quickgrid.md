...
* <xref:Microsoft.AspNetCore.Components.QuickGrid.QuickGrid%601.AnchorMode%2A>: Only applicable when using <xref:Microsoft.AspNetCore.Components.QuickGrid.QuickGrid%601.Virtualize%2A>. Controls how the viewport behaves at list edges when items are dynamically added (default: `Start`). This forwards to the inner `Virtualize` component. For more information, see <xref:blazor/components/virtualization#control-viewport-scroll-position-behavior-when-items-are-dynamically-added>.
* <xref:Microsoft.AspNetCore.Components.QuickGrid.QuickGrid%601.ItemComparer%2A>: Only applicable when using <xref:Microsoft.AspNetCore.Components.QuickGrid.QuickGrid%601.Virtualize%2A>. A comparer used to detect whether items were prepended or appended between data loads, which is useful for class-typed items supplied by an <xref:Microsoft.AspNetCore.Components.QuickGrid.QuickGrid%601.ItemsProvider%2A>. This forwards to the inner `Virtualize` component. For more information, see <xref:blazor/components/virtualization#control-viewport-scroll-position-behavior-when-items-are-dynamically-added>.
:::moniker-end
